...
SLURM_GPUS_PER_TASK=4  # sets 4 GPUs per task (Ray worker)
...
srun --nodes=1 --ntasks=1 --overlap -w "$head_node" vllm serve /checkpoints/your_username/rankmind/rankmind_llama3_70b_r1_grpo_megatron_answer_agent/global_step_80/actor/huggingface \
  --trust-remote-code \
  --seed=1 \
  --port 8003 \
  --served-model-name "llama3_70b_r1_grpo_megatron_answer_agent" \
