...
# srun --nodes=1 --ntasks=1 --overlap -w "$head_node" vllm serve /checkpoints/your_username/rankmind/rankmind_llama3_70b_sft_grpo_megatron_code_agent/global_step_10/actor/huggingface \
srun --nodes=1 --ntasks=1 --overlap -w "$head_node" vllm serve /checkpoints/your_username/rankmind/llama3_70b_code_gpt_o4_sft/checkpoint-80 \
  --trust-remote-code \
  --seed=1 \
  --port 8002 \
  --served-model-name "llama3_70b_code_gpt_o4_sft" \
