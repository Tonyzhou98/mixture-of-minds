...
# srun --nodes=1 --ntasks=1 --overlap -w "$head_node" vllm serve /checkpoints/your_username/rankmind/llama3_70b_answer_gpt_o4_sft/checkpoint-70 \
srun --nodes=1 --ntasks=1 --overlap -w "$head_node" vllm serve /checkpoints/your_username/rankmind/rankmind_llama3_70b_sft_grpo_megatron_answer_agent/global_step_95/actor/huggingface \
  --trust-remote-code \
  --seed=1 \
  --port 8003 \
  --served-model-name "llama3_70b_answer_gpt_o4_sft" \
